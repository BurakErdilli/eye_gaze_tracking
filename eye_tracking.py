...
def get_gaze_ratio_updown(eye_points, facial_landmarks):
    ...
    if gaze_ratio==1:
        ...
        cv2.putText(frame, "Down "+str("%.2f" % gaze_ratio), (400, 400), font, 2, (0, 255, 0), 3)

    elif(gaze_ratio<0.25):
        cv2.putText(frame, "UP "+str("%.2f" % gaze_ratio), (800, 400), font, 2, (255, 0, 0), 3)
...
while True:
    ...
    for face in faces:
        ...
        gaze_ratio_right_eye_updown = get_gaze_ratio_updown([36, 37, 38, 39, 40, 41], landmarks)
        gaze_ratio_left_eye_updown = get_gaze_ratio_updown([42, 43, 44, 45, 46, 47], landmarks)
        gaze_ratio = (gaze_ratio_right_eye + gaze_ratio_left_eye) / 2

        if gaze_ratio <= 0.65:
            cv2.putText(frame, "RIGHT "+str("%.2f" % gaze_ratio), (50, 100), font, 2, (0, 0, 255), 3)
            new_frame[:] = (0, 0, 255)
        elif 0.65< gaze_ratio <= 1.8:
            cv2.putText(frame, "CENTER "+str("%.2f" % gaze_ratio), (50, 100), font, 2, (0, 0, 255), 3)


        elif 1.8 < gaze_ratio <= 5:
            new_frame[:] = (255, 0, 0)
            cv2.putText(frame, "LEFT "+str("%.2f" % gaze_ratio), (50, 100), font, 2, (0, 0, 255), 3)
        else: 
            pass
